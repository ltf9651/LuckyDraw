...
func resetAllGiftPrizeData() {
	...
	for _, giftInfo := range list {
		if giftInfo.PrizeTime != 0 &&
			...
			// 立即执行
			log.Println("crontab start util.ResetGiftPrizeData giftInfo=", giftInfo)
			util.ResetGiftPrizeData(&giftInfo, giftService)
			// 预加载缓存数据
			giftService.GetAll(true)
			log.Println("crontab end util.ResetGiftPrizeData giftInfo")
		}
	}
...
func distributionAllGiftPool() {
	log.Println("crontab start util.DistributionGiftPool")
	num := util.DistributionGiftPool()
	log.Println("crontab end util.DistributionGiftPool, num=", num)
